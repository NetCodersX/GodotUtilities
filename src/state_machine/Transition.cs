...
public partial class Transition<T> where T : Enum
{
    ...
    public float? MinTimeOverride { get; private set; }
    ...
    internal bool IsMinTimeExceeded(float time, float fallbackMinTime)
    {
        float required = MinTimeOverride.HasValue ? MinTimeOverride.Value : fallbackMinTime;
