...
public partial class InventoryManager : Node, ISaveable
{
    ...
    private void OnPanelOpened(StringName panelName)
    {
        ...
        if (panel is not ExternalInventoryPanel external) return;

        if (external.Data is null)
        {
            Log.Error("Inventory", "ExternalInventoryPanel opened without data assigned. Call SetInventoryData before opening.");
            return;
        }
        ...
        external.Attach(PlayerInventory, ExternalData);
    }
